...
public:
  ...
  void collectTableCell(unsigned row, unsigned column, const boost::optional<std::string> &content, unsigned rowSpan, unsigned columnSpan,  const boost::optional<IWORKFormula> &formula, const bool formulaCell);
  void collectCoveredTableCell(unsigned row, unsigned column);
  void collectTableRow();
  void collectTable();
  void collectTableNameMap(const IWORKTableNameMapPtr_t &tableNameMap);
