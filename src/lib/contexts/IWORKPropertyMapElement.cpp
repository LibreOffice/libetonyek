...
class SFTStrokePropertyElement : public PropertyContextBase
{
public:
...
IWORKXMLContextPtr_t SFTStrokePropertyElement::element(const int name)
{
  m_default = false;
  switch (name)
  {
...
void SFTStrokePropertyElement::endOfElement()
{
  if (!m_default)
    m_propMap.put<property::SFTStrokeProperty>(get(m_value));
  else
    m_propMap.clear<property::SFTStrokeProperty>();
}
