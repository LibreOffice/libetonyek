...
{
public:
  StylePropertyElement(IWORKXMLParserState &state, IWORKStyleMap_t *style, IWORKPropertyMap &propMap);
...
StylePropertyElement::StylePropertyElement(IWORKXMLParserState &state, IWORKStyleMap_t *style, IWORKPropertyMap &propMap)
  : PropertyContextBase(state, propMap)
  , m_style(style)
{
}
...
IWORKXMLContextPtr_t StylePropertyElement::element(const int name)
{
  ...
  case IWORKToken::NS_URI_SF | IWORKToken::paragraphstyle :
    return makeContext<IWORKStyleContext>(getState(), m_style, true);
...
IWORKXMLContextPtr_t IWORKPropertyMapElement::element(const int name)
{
  ...
  case IWORKToken::NS_URI_SF | IWORKToken::SFTCellStylePropertyParagraphStyle :
    return makeContext<StylePropertyElement>(getState(), &getState().getDictionary().m_paragraphStyles, m_propMap);
