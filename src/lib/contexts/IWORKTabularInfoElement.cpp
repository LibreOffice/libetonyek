...
{
public:
  explicit StyleRunElement(IWORKXMLParserState &state, IWORKGridLineList_t &gridLines, bool verticalLines);
...
StyleRunElement::StyleRunElement(IWORKXMLParserState &state, IWORKGridLineList_t &gridLines, bool verticalLines)
  : IWORKXMLElementContextBase(state)
  , m_gridLines(gridLines)
  , m_verticalLines(verticalLines)
  , m_line(0, 0, IWORKStylePtr_t())
{
}

void StyleRunElement::startOfElement()
{
  if (m_verticalLines)
    m_line = IWORKGridLine_t(0, getState().m_tableData->m_numRows, IWORKStylePtr_t());
  else
    m_line = IWORKGridLine_t(0, getState().m_tableData->m_numColumns, IWORKStylePtr_t());
}
...
IWORKXMLContextPtr_t VerticalGridlineElement::element(const int name)
{
  ...
  case IWORKToken::style_run | IWORKToken::NS_URI_SF :
    return makeContext<StyleRunElement>(getState(), getState().m_tableData->m_verticalLines, true);
  }
