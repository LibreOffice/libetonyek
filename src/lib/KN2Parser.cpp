...
KNPosition readPosition(const KNXMLReader &reader)
{
  ...
  while (attr.next())
  {
    if (KN2Token::NS_URI_SFA == getNamespaceId(attr))
    {
      ...
      {
      case KN2Token::x :
        position.x = asDouble(attr.getValue());
        break;
      case KN2Token::y :
        position.y = asDouble(attr.getValue());
        break;
      default :
        KN_DEBUG_XML_UNKNOWN("attribute", attr.getName(), attr.getNamespace());
        break;
...
void KN2Parser::parsePropertyMap(const KNXMLReader &reader, KNStyle &style)
{
...
ID_t KN2Parser::parseStyle(const KNXMLReader &reader, KNStyle &style)
{
  ID_t id;
