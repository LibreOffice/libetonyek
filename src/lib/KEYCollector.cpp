...
void KEYCollector::collectStickyNote()
{
  ...
  {
    m_stickyNotesProps.insert("svg:x", pt2in(m_levelStack.top().m_geometry->m_position.m_x));
    m_stickyNotesProps.insert("svg:y", pt2in(m_levelStack.top().m_geometry->m_position.m_y));
    m_stickyNotesProps.insert("svg:width", pt2in(m_levelStack.top().m_geometry->m_naturalSize.m_width));
    m_stickyNotesProps.insert("svg:height", pt2in(m_levelStack.top().m_geometry->m_naturalSize.m_height));
  }
...
void KEYCollector::drawStickyNotes()
{
  ...
  m_document->openComment(m_stickyNotesProps);
