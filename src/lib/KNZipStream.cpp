...
private:
  KNZipStreamImpl(const KNZipStreamImpl &);
  KNZipStreamImpl &operator=(const KNZipStreamImpl &);
...
KNZipStream::~KNZipStream()
{
  if (m_pImpl)
    delete m_pImpl;
}

const unsigned char *KNZipStream::read(unsigned long numBytes, unsigned long &numBytesRead)
{
  return m_pImpl->m_input->read(numBytes, numBytesRead);
}

int KNZipStream::seek(long offset, WPX_SEEK_TYPE seekType)
{
  return m_pImpl->m_input->seek(offset, seekType);
}

long KNZipStream::tell()
{
  return m_pImpl->m_input->tell();
}

bool KNZipStream::atEOS()
{
  return m_pImpl->m_input->atEOS();
}

bool KNZipStream::isOLEStream()
{
  return m_pImpl->isZipStream();
}

WPXInputStream *KNZipStream::getDocumentOLEStream(const char *name)
{
...
bool KNZipStreamImpl::findCentralDirectoryEnd()
{
...
bool KNZipStreamImpl::isZipStream()
{
  if (m_cdir_offset)
  {
    if(m_cdir.empty())
...
bool KNZipStreamImpl::readCentralDirectory(const CentralDirectoryEnd &end)
{
  try
  {
...
WPXInputStream *KNZipStreamImpl::getSubstream(const char *name)
{
  if (m_cdir.empty())
    return 0;
  ...
  if (!entry.compression)
    return new KNMemoryStream(m_input, entry.compressed_size);
  else
  {
    int ret;
    z_stream strm;
    ...
    (void)inflateEnd(&strm);
    return new KNMemoryStream(data);
  }
}

bool KNZipStreamImpl::readCentralDirectoryEnd(CentralDirectoryEnd &end)
{
  try
  {
    unsigned signature = readU32(m_input);
...
bool KNZipStreamImpl::readLocalFileHeader(LocalFileHeader &header)
{
  try
  {
    unsigned signature = readU32(m_input);
...
bool KNZipStreamImpl::areHeadersConsistent(const LocalFileHeader &header, const CentralDirectoryEntry &entry)
{
